...
function refs(g){
  ...
  for(const e of g.events){if(e.type==='sub'&&e.swap===undefined)add(e.id);for(const k of ['pitcher','fielder','putout','errorFielder','thirdOutId','runnerId','personId','replacementId'])add(e[k]);for(const a of e.actions||[]){add(a.id);add(a.putout);add(a.errorFielder);}}
  return set;
}
export function exportArchive(db,selectedYears){
  const games=db.games.filter(g=>g.ended&&selectedYears.includes(gameYear(g))).map(g=>({...clone(g),archiveId:archiveId(g),draft:null,pendingPitch:null}));
...
export function importArchive(db,text){
  ...
  for(const raw of data.games){
    ...
    for(const e of g.events){if(e.type==='sub'&&e.swap===undefined)remap(e,'id');for(const k of ['pitcher','fielder','putout','errorFielder','thirdOutId','runnerId','personId','replacementId'])remap(e,k);for(const a of e.actions||[]){remap(a,'id');remap(a,'putout');remap(a,'errorFielder');}}
    const lineup=g.teams.flatMap(t=>t.lineup.map(p=>p.id));if(new Set(lineup).size!==lineup.length)throw Error('同名映射导致阵容重复，请核对球员姓名');
    g.id=uid();g.draft=null;g.pendingPitch=null;g.rulesVersion=2;
    const state=replay(g);for(const st of Object.values(state.stats))for(const v of Object.values(st))if(!Number.isFinite(v)||v<0)throw Error('比赛统计无效');
